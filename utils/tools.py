...
def get_buttons():
    ...
    for config in CONFIGURABLE:
        BTN.append(
            [
                types.InlineKeyboardButton(CONFIGURABLE[config]['name'], callback_data=f"settings_info#{config}"),
                types.InlineKeyboardButton(better_bool(getattr(Config, config)), callback_data=f'settings_set#{config}')
            ]
            )
    return BTN
...
async def format_buttons(files: list):
    if Config.CHANNEL:
        btn = [
            [
...
async def check_fsub(bot: Client, message: types.Message, try_again: str = None):
    ...
    try:
        ...
        btn = [
                [types.InlineKeyboardButton("Join Update Channel", url=invite_link.invite_link)],
            ]
        if try_again:
            btn.append([types.InlineKeyboardButton('Try Again', url=f'https://t.me/{bot.me.username}?start={try_again}')])
        await message.reply("Join our update channel to use this bot.", reply_markup=types.InlineKeyboardMarkup(btn))
        return False
    else:
