...
lock = asyncio.Lock()
_REGEX = "(https://)?(t\.me/|telegram\.me/|telegram\.dog/)(c/)?(\d+|[a-zA-Z_0-9]+)/(\d+)$"
...
async def index_files(bot: Client, query: types.CallbackQuery):
    ...
    _, sts, chat, lst_msg_id, from_user = query.data.split("#")
    if sts == 'reject':
...
@Client.on_message(((filters.forwarded & ~filters.text) | (filters.regex(_REGEX)) & filters.text) & filters.private & filters.incoming & filters.user(Config.ADMINS))
...
async def index_files_to_db(lst_msg_id: int, chat: int, msg: types.Message, bot: Bot):
    ...
    async with lock:
        try:
            ...
            await msg.edit(f'Successfully saved <code>{total_files} / {current}</code> to dataBase!\nDuplicate Files Skipped: <code>{duplicate}</code>\nDeleted Messages Skipped: <code>{deleted}</code>\nNon-Media messages skipped: <code>{no_media + unsupported}</code>(Unsupported Media - `{unsupported}` )\nErrors Occurred: <code>{errors}</code>')
